...
async def async_setup_entry(
        ...
        try:
            ...
            for fn in candidates:
                try:
                    ...
                    if ok_any:
                        _LOGGER.debug(
                            "[MQTT][battery] Renouvellement streaming OK via %s",
                            getattr(fn, "__name__", fn),
                        )
                        return
                except Exception as e:
                    _LOGGER.debug(
                        "[MQTT][battery] Hook %s a échoué: %s",
                        getattr(fn, "__name__", fn),
                        e,
                    )
            _LOGGER.warning(
                "[MQTT][battery] Impossible de renouveler le streaming avec les hooks testés."
            )
        except Exception as e:
            _LOGGER.warning(
                "[MQTT][battery] Erreur lors du renouvellement du streaming: %s", e
            )
    ...
    async def _start_after_start(_: object = None) -> None:
        ...
        renew_streaming_for_entry = functools.partial(
            _renew_battery_streaming, entry=entry
        )
        tick_availability_for_entry = functools.partial(_tick_availability, entry=entry)
        await renew_streaming_for_entry()
        cancel_renew = async_track_time_interval(
            hass, renew_streaming_for_entry, timedelta(seconds=90)
        )
